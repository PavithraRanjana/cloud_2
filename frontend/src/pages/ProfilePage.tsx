...
function AdminProfile({ user, logout, navigate }: {
  user: { full_name?: string; username?: string; email?: string; role?: string } | null;
  logout: () => void;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            <p className="text-lg font-bold text-gray-900">{user?.full_name ?? user?.username}</p>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-0.5 text-xs font-bold text-blue-700 uppercase tracking-wide mt-1">
              Administrator
            </span>
          </div>
        </div>

        <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm border-t pt-4">
          {[
      ...
      {/* Permissions */}
      <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
        <h3 className="font-bold text-gray-900 mb-4">Permissions</h3>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            "Manage flight pricing",
          ].map(p => (
            <div key={p} className="flex items-center gap-2 rounded-lg bg-emerald-50 border border-emerald-100 px-3 py-2 text-sm text-emerald-700">
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          {[
            ...
            <div key={p} className="flex items-center gap-2 rounded-lg bg-gray-50 border border-gray-100 px-3 py-2 text-sm text-gray-400">
              <svg className="w-4 h-4 shrink-0" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="rounded-2xl bg-white border border-red-100 shadow-sm p-6">
        ...
        <p className="text-xs text-gray-400 mb-4">Permanently delete this admin account. This action cannot be undone.</p>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-500">This will permanently delete your account. You will be logged out immediately.</p>
            <p className="mt-4 text-sm font-medium text-gray-700">Type <span className="font-mono font-bold text-red-600">DELETE</span> to confirm:</p>
            ...
            <div className="mt-4 flex gap-3">
              <button onClick={() => { setShowDeleteModal(false); setDeleteConfirm(""); }}
                className="flex-1 rounded-lg border border-gray-200 py-2 text-sm font-medium hover:bg-gray-50 transition-colors">Cancel</button>
...
export function ProfilePage() {
  ...
  if (user?.role === "admin") return <AdminProfile user={user} logout={logout} navigate={navigate} />;
